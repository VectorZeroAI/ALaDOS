...
CREATE TABLE IF NOT EXISTS knowledge (
    addr BIGINT DEFAULT new_addr() PRIMARY KEY REFERENCES addrs(addr)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS master_context (
    addr BIGINT PRIMARY KEY REFERENCES masters(addr)
        ON DELETE CASCADE
        ON UPDATE CASCADE,
    window_anchor_exe BIGINT REFERENCES executables(addr)
        ON DELETE SET NULL
        ON UPDATE CASCADE,
    window_anchor_knowledge BIGINT REFERENCES knowledge(addr)
        ON DELETE SET NULL
        ON UPDATE CASCADE,
    window_size_r INT,
    window_size_l INT,
...
CREATE TABLE IF NOT EXISTS slave_req (
    slave_addr BIGINT REFERENCES slaves(addr)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
    req_addr BIGINT REFERENCES results(addr)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ownership(
    addr BIGINT PRIMARY KEY REFERENCES addrs(addr)
        ON UPDATE CASCADE
        ON DELETE CASCADE,
